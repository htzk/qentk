...
from base.model_class import count_obs
...
class DataGen:
    ...
    def gen_data(self, noise_level=1e-4):
        ...
        yb_list_l=[]
        yb_list_t=[]
        for i in range(len(y_list_l)):
            if y_list_l[i] < (1. + noise_l[i])*0.5*self.nqubit: 
                yb_list_l.append(0.)
            else:
                yb_list_l.append(1.)

        for i in range(len(y_list_t)):
            if y_list_t[i] < (1. + noise_t[i])*0.5*self.nqubit: 
                yb_list_t.append(0.)
            else:
                yb_list_t.append(1.)

        return torch.tensor(self.x_in[0:self.M], dtype=torch.float32), yb_list_l, y_list_l, \
               torch.tensor(self.x_in[self.M:self.M+self.Mt], dtype=torch.float32), yb_list_t, y_list_t, self.encoder
